    ...
    switch (ttlmode->vmode)
    {
        ...
        case MDA:   // pal = (pal != PAL_MDA) ? PAL_MDA : PAL_EGA;
                    ...
                    if (ttlmode->xscanlrd != hscan0) {
                        _toggle_hscan = 1;
                        float d0 = ttlmode->div_int + (ttlmode->div_frac+0.5f)/256.0f;  // 720:(3,42) <-> 640:(3,143)
...
int main() {
    ...
    // 114.0 MHz / 228 MHz  ->  70.1375Hz / 31.49kHz (720x400)
    uint vco_freq = 1596000000;
    uint post_div1 = 7;
    uint post_div2 = 1; // 2: 114MHz , 1: 228MHz -> vga_out: div2
    ...
    while (1)
    {
        ...
        if (cnt == 10) {  // 5-6 Hz
            ...
            if (gpio_get(BUTTON_OSD) == 0) {
                ...
                // scan: cycles_per_pixel=4
                // CLKkHZ/1e3 / 4.0 = 228.0/4=57.0
                float pxclk = ((float)CLKkHZ/1e3/4.0)/(ttlmode->div_int + ttlmode->div_frac/256.0f);
